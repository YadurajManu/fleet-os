...
before(async () => {
  ...
  const [s] = await ctx.db
    .insert(services)
    .values({ fleetId, name: 'app', buildContext: '.' })
    .returning()
  serviceId = s!.id
  const [n] = await ctx.db
    ...
    .values({
      ...
      ramMb: 16384,
      effectiveCpu: 8,
      effectiveMemBytes: 16 * 1073741824,
      diskMb: 100000,
      buildCacheFreeBytes: 100 * 1073741824,
      canBuild: true,
      status: 'online',
      agentTokenHash: 'test-build-hash',
    })
    .returning()
  nodeId = n!.id
...
test('assignment has scoped credentials; wrong attempts and nodes cannot finish it; result uses digest', async () => {
  ...
  ctx.tunnels.sendBuild = (id, msg) => {
    const a = msg as BuildAssignment
    if (a.type !== 'build.assign') return true
    task = (async () => {
      assert.equal(id, nodeId)
      assert.equal(a.platform, 'linux/arm64')
      assert.equal(
        verifyGrant(a.registry_password, ctx.config.JWT_SECRET, 'push')?.repo,
        `fleet-builds/${serviceId}`
      )
      assert.equal(
        verifyGrant(a.source.token, ctx.config.JWT_SECRET, 'push'),
        null
      )
      const event = {
        type: 'build.result',
      ...
      const [pending] = await ctx.db
        .select()
        .from(buildJobs)
        .where(eq(buildJobs.id, a.job_id))
      assert.equal(pending!.status, 'assigned')
      await runner.handleMessage(id, {
        ...event,
        type: 'build.ack',
        status: 'running',
      })
  ...
  const result = await runner.build(req)
  await task
  assert.equal(
    result.imageTags[0],
    `registry.example.test/fleet-builds/${serviceId}@${digest}`
  )
})
test('same source, service and platform reuse a successful digest', async () => {
  ctx.tunnels.sendBuild = () => {
    throw new Error('dedupe unexpectedly assigned a build')
  }
...
test('no opted-in builder fails clearly and leaves no active jobs', async () => {
  ...
  const jobs = await ctx.db
    .select()
    .from(buildJobs)
    .where(eq(buildJobs.deploymentId, req.deploymentId!))
  assert.ok(
    jobs.every((j) => !['queued', 'assigned', 'running'].includes(j.status))
  )
  await ctx.db
    .update(nodes)
    .set({ buildCacheFreeBytes: 100 * 1073741824, canBuild: true })
    .where(eq(nodes.id, nodeId))
})
test('operator cancellation stops the assigned attempt and returns a terminal error', async () => {
  ...
  ctx.tunnels.sendBuild = (id, msg) => {
    const a = msg as BuildAssignment
    if (a.type === 'build.assign') void runner.cancel(req.deploymentId!)
    else if ((msg as { type: string }).type === 'build.cancel')
      cancelSent = true
    return true
  }
...
test('expired lease retries on another native builder; late result is fenced', async () => {
  const [n] = await ctx.db
    ...
    .values({
      ...
      canBuild: true,
      status: 'online',
      agentTokenHash: 'test-build-hash-two',
    })
    .returning()
  const assigned: string[] = []
  let previous: BuildAssignment | undefined
  let task: Promise<void> | undefined
  ctx.tunnels.sendBuild = (id, msg) => {
    const a = msg as BuildAssignment
    if (a.type !== 'build.assign') return true
    ...
    task = (async () => {
      ...
      await runner.handleMessage(assigned[0]!, {
        type: 'build.result',
        version: 1,
        job_id: a.job_id,
  ...
  const result = await runner.build(await request('retry'))
  await task
  assert.equal(result.digest, digest)
...
test('agent builds are the default, local remains an explicit fallback', () => {
  assert.equal(
    loadConfig({ ...process.env, BUILD_MODE: undefined }).BUILD_MODE,
    'agent'
  )
...
test('a cancelled deployment cannot start a later build', async () => {
  ...
  await ctx.db
    .update(deployments)
    .set({ status: 'failed', failureReason: 'build_cancelled' })
    .where(eq(deployments.id, req.deploymentId!))
  ctx.tunnels.sendBuild = () => {
    throw new Error('assigned a cancelled deployment')
  }
  await assert.rejects(runner.build(req), /cancelled/)
})
