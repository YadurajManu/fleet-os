...
import { planPlatforms, selectBuilder, type Builder } from '../src/build/platforms.js'
import { filterNodes } from '../src/scheduler/placement.js'
import type { NodeSnapshot, ServiceSpec } from '../src/scheduler/types.js'
const node = (id: string, arch: string): NodeSnapshot => ({id,name:id,arch,platform:`linux/${arch}`,status:'online',ramMb:8192,cpuCores:8,hasGpu:false,reliabilityTier:'standard',tags:[],committedRamMb:0})
const service: ServiceSpec = {id:'s',name:'s',placementPolicy:'flexible',requestRamMb:256,requestCpu:0.25,requiresGpu:false,minReliabilityTier:'standard',compatibleArches:[],affinity:[],antiAffinity:[],persistentVolume:false}
test('auto builds all eligible platforms, including failover nodes',()=>{
 const nodes=[node('mac','arm64'),node('linux','amd64'),{...node('offline','armv7'),status:'offline' as const}]
 assert.deepEqual(planPlatforms(service,nodes),['linux/amd64','linux/arm64'])
 assert.deepEqual(planPlatforms({...service,placementArch:'arm64'},nodes),['linux/arm64'])
})
test('manifest platform mismatch needs explicit emulation opt-in',()=>{
 ...
 assert.deepEqual(filterNodes(spec,pool).eligible.map(n=>n.id),['linux'])
 assert.equal(filterNodes({...spec,allowEmulation:true},pool).eligible.length,2)
 assert.deepEqual(filterNodes({...spec,allowEmulation:true,placementArch:'arm64'},pool).eligible.map(n=>n.id),['mac'])
})
...
test('native selection, warm cache, opt-in, concurrency, and explicit QEMU fallback',()=>{
 ...
 assert.equal(selectBuilder([{...mac,canBuild:false}],'linux/arm64',null),undefined)
