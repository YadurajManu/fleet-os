...
export function archivePath(workdir: string, deploymentId: string): string {
  return join(workdir, 'delegated', `${deploymentId}.tgz`)
}
async function activeGrant(ctx: AppContext, grant: BuildGrant | null) {
  ...
  const [job] = await ctx.db
    .select()
    .from(buildJobs)
    .where(eq(buildJobs.id, grant.job))
    .limit(1)
  return job &&
    job.builderNodeId === grant.node &&
    job.attempt === grant.attempt &&
    ['assigned', 'running'].includes(job.status) &&
    job.leaseExpiresAt &&
    job.leaseExpiresAt.getTime() > Date.now() &&
    grant.repo === `fleet-builds/${job.serviceId}`
    ? job
    : null
}
...
export async function buildTransferRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { jobId: string } }>(
    ...
    async (req, reply) => {
      ...
      if (!job || job.id !== req.params.jobId)
        return reply
          .code(403)
          .send({ error: 'source grant expired or invalid' })
      return reply
        .header('Cache-Control', 'no-store')
        .type('application/gzip')
        .send(
          createReadStream(
            archivePath(ctx.config.BUILD_WORKDIR, job.deploymentId)
          )
        )
    }
  )
  ...
  await app.register(async (registry) => {
    ...
    registry.addContentTypeParser('*', (req, payload, done) =>
      done(null, payload)
    )
    registry.all('/v2/*', async (req, reply) => {
      ...
      if (!job || !grant)
        return reply
          .header('WWW-Authenticate', 'Basic realm="Fleet build registry"')
          .code(401)
          .send()
      const path = new URL(req.raw.url ?? '/', 'http://registry.local')
      if (
        path.pathname !== '/v2/' &&
        !path.pathname.startsWith(`/v2/${grant.repo}/`)
      )
        return reply.code(403).send()
      if (
        path.pathname.includes('%') ||
        (req.method === 'DELETE' && !path.pathname.includes('/blobs/uploads/'))
      )
        return reply.code(403).send()
      // Cross-repository blob mounts would bypass the repository scope.
      if (path.searchParams.has('from') || path.searchParams.has('mount'))
        return reply.code(403).send()
      if (!ctx.config.REGISTRY_URL) return reply.code(503).send()
      const origin = new URL(
        ctx.config.REGISTRY_URL.includes('://')
          ? ctx.config.REGISTRY_URL
          : `https://${ctx.config.REGISTRY_URL}`
      )
      ...
      if (ctx.config.REGISTRY_CREDENTIALS)
        headers.authorization = `Basic ${Buffer.from(ctx.config.REGISTRY_CREDENTIALS).toString('base64')}`
      const upstream = (
        target.protocol === 'https:' ? httpsRequest : httpRequest
      )(target, { method: req.method, headers, timeout: 300000 })
      return await new Promise<void>((resolve, reject) => {
        upstream.on('timeout', () =>
          upstream.destroy(new Error('registry request timed out'))
        )
        upstream.on('error', () => {
          if (!reply.sent)
            reply.code(502).send({ error: 'upstream registry unavailable' })
          resolve()
        })
        ...
        upstream.on('response', (res) => {
          ...
          if (res.headers.location) {
            ...
            if (
              ...
              !location.pathname.startsWith(`/v2/${grant.repo}/`)
            ) {
              res.resume()
              reply
                .code(502)
                .send({ error: 'unsupported upstream registry redirect' })
              resolve()
              return
            }
