...
async function withRegistry<T>(host: string, credentials: string | undefined, action: (env: NodeJS.ProcessEnv) => Promise<T>): Promise<T> {
  ...
  try {
    if (credentials) {
      ...
      await new Promise<void>((resolve, reject) => {
        const child = spawn('docker', ['login', host, '--username', credentials.slice(0, separator), '--password-stdin'], { env, stdio: ['pipe', 'ignore', 'ignore'], timeout: 30000 })
        child.once('error', reject)
        child.stdin.once('error', reject)
        child.once('exit', code => code === 0 ? resolve() : reject(new Error('registry authentication failed')))
...
export async function mergeManifests(repo: string, tag: string, digests: string[], credentials?: string): Promise<string> {
  ...
  try {
    return await withRegistry(repo.split('/')[0]!, credentials, async env => {
      ...
      if (!/^sha256:[a-f0-9]{64}$/.test(digest)) throw new Error('manifest digest missing')
      return digest
    })
...
export async function inspectImage(image: string, config: RegistryConfig = {}): Promise<{ platforms: string[]; digest: string }> {
  if (image.startsWith('-') || /[\s\r\n]/.test(image)) throw new Error('invalid image reference')
  const registry = config.REGISTRY_URL?.replace(/^https?:\/\//, '').split('/')[0]
  // Never send the private registry's credentials to an arbitrary image host.
  const host = image.split('/')[0]!
  const credentials = registry && host === registry ? config.REGISTRY_CREDENTIALS : undefined
  try {
    return await withRegistry(host, credentials, async env => {
      const { stdout } = await exec('docker', ['buildx', 'imagetools', 'inspect', '--raw', image], { env, timeout: 30000, maxBuffer: 4 * 1048576 })
      const manifest = JSON.parse(stdout)
      let platforms = manifestPlatforms(manifest)
      if (!manifest.manifests) {
        const { stdout: config } = await exec('docker', ['buildx', 'imagetools', 'inspect', '--format', '{{json .Image}}', image], { env, timeout: 30000, maxBuffer: 4 * 1048576 })
      ...
      const { stdout: d } = await exec('docker', ['buildx', 'imagetools', 'inspect', '--format', '{{.Manifest.Digest}}', image], { env, timeout: 30000, maxBuffer: 1024 })
      const digest = d.trim()
      if (!/^sha256:[a-f0-9]{64}$/.test(digest)) throw new Error('no image digest')
