...
export default function SignIn() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (mode === 'in') {
        ...
        if (res && res.requires2fa && res.challengeToken) {
  ...
  async function submitTotp(e: React.FormEvent) {
    ...
    setTotpError(null)
    try {
      await complete2fa(challengeToken, totpCode.trim())
    } catch (err) {
      setTotpError(err)
    } finally {
      setTotpBusy(false)
        ...
        <div className="w-full max-w-[380px] fade-up">
          ...
          {challengeToken ? (
            <div className="mt-10 fade-up">
              <div className="flex h-11 w-11 items-center justify-center rounded-[4px] border border-[color-mix(in_oklab,var(--color-signal)_30%,transparent)] bg-[color-mix(in_oklab,var(--color-signal)_10%,transparent)] text-[var(--color-signal)]">
              ...
              <h1 className="mt-6 text-[26px] font-semibold leading-tight tracking-[-0.035em]">
              ...
              <form onSubmit={submitTotp} className="mt-8 space-y-5">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-[3px] border border-[var(--color-line-2)] bg-[var(--color-ink-900)] px-4 py-3 font-mono text-[24px] tracking-[0.35em] text-[var(--color-fg)] text-center transition-colors focus:border-[var(--color-signal)] focus:outline-none"
                  />
                </div>
                ...
                >
                  {totpBusy ? 'verifying…' : 'Verify & Continue'}
                </Button>
                ...
                <button
                  ...
                  className="w-full text-center font-mono text-[11.5px] text-[var(--color-fg-dim)] transition-colors hover:text-[var(--color-fg-muted)]"
                >
